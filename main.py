...
def get_weather(city_tf, parent):
    ...
    if city_name:
        ...
        if weather_data.status_code == 200:
            ...
            Label(weather_window, text=f"{temp_c}°C", font=("Arial", 50), background=bg_color, fg='black').grid(row=0, column=0, padx=20, pady=20)
            Label(weather_window,
                  text=f"Ощущается как: {feel_c}°C\nВлажность: {humidity}%\nДавление: {pressure} hPa\nСкорость ветра: {wind_speed} м/с",
                  font=("Arial", 14), background=bg_color, fg='black').grid(row=0, column=1, columnspan=1, padx=20, pady=20)
            ...
            if forecast_data.status_code == 200:
                ...
                tk.Button(weather_window, text='Сохранить график', command=lambda: save_graph(fig)).grid(row=2, columnspan=2, pady=10)
            else:
...
tk.Button(tab_current, text='Найти', command=lambda: get_weather(city_tf_current, root)).grid(row=0, column=2, padx=5, pady=5)
...
tk.Button(tab_std, text="Построить графики", command=lambda: on_submit_std(city_entry_std, start_date_entry_std, end_date_entry_std, frequency_combobox_std)).grid(row=4, columnspan=2, pady=10)
...
tk.Button(tab_avg, text="Построить графики", command=lambda: on_submit_avg(city_entry_avg, start_date_entry_avg, end_date_entry_avg, frequency_combobox_avg)).grid(row=4, columnspan=2, pady=10)
